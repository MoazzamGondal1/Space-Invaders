...
void boxes(int box_x, int box_y, Dimensions box[5][10], int win_y) {
	int a = box_x;
	for (int i = 0; i < 5; i++) {
		for (int j = 0; j < 10; j++) {
			if (box_y < win_y - 50)
...
void D_boxes(int box_x, int box_y, Dimensions box[5][10], int win_y) {
	int a = box_x;
	for (int i = 0; i < 5; i++) {
		for (int j = 0; j < 10; j++) {
			if (box_y < win_y - 50)
...
void BoxDimensions(int box_x, int box_y, Dimensions B[5][10]) {
	for (int i = 0; i < 5; i++) {
		for (int c = 0; c < 10; c++) {
			B[i][c].x1 = box_x;
			B[i][c].y1 = box_y;
